...
from get_location import get_photo_location, _extract_photo_id
...
classes_to_scrape = []
classes_scraped_before = list(map(lambda s: s.replace('_', ' '), os.listdir(imagenet_images_folder)))
   ...
   for item in args.class_list:
    ...
    for key, val in class_info_dict.items():
        ...
        if args.scrape_only_flickr:
            if int(val['flickr_img_url_count']) * 0.9 > args.images_per_class and val['class_name'] not in \
                    classes_scraped_before:
...
def get_image(img_url):
    ...
    img_file_path = os.path.join(class_folder, img_name.split("_")[0]) + '.jpg'
    logging.debug(f'Saving image in {img_file_path}')
    photo_id = _extract_photo_id(img_url)
    result = None
    if photo_id is not None:
        _, result = get_photo_location(photo_id)

    if result is not None:
        result_file = open(metadata_path, 'a+', encoding="utf-8")
        print(img_url)
        img = Image.open(BytesIO(img_resp.content)).resize((224, 224))
        img.save(img_file_path)
        writer = csv.DictWriter(result_file, fieldnames=['id', 'lat', 'lon', 'locality', 'country', 'region', 'county'],
                                lineterminator='\n')
...
for class_wnid in classes_to_scrape:
    ...
    class_folder = os.path.join(imagenet_images_folder, class_name.replace(" ", "_"))
